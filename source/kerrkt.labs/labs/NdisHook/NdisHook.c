...
PVOID FindTcpIpProtcolBlock(PVOID ProtocolBlock)
{
    ...
    while (ProtocolBlock)
    {
        if (RtlEqualUnicodeString(&TcpIp, NDIS_PROTOCOL_BLOCK_NAME(ProtocolBlock), TRUE))
        {
            return ProtocolBlock;
        }

        ProtocolBlock = *NDIS_PROTOCOL_BLOCK_NEXT(ProtocolBlock);
    }
...
VOID UpdateProtocolOpenBlockRecvHandler(PVOID ProtocolBlock, PVOID OrgHandler, PVOID NewHandler)
{
    ...
    //DbgPrint("[LAB] OrgHandler: (%p) NewHandler (%p)\n", OrgHandler, NewHandler);

    while (OpenBlock)
    {
        //DbgPrint("[LAB] Open Block Handler: (%p)\n", *NDIS_OPEN_BLOCK_RECVNBL(OpenBlock));
        if (*NDIS_OPEN_BLOCK_RECVNBL(OpenBlock) == OrgHandler)
        {
            *NDIS_OPEN_BLOCK_RECVNBL(OpenBlock) = NewHandler;
        }

        OpenBlock = *NDIS_OPEN_BLOCK_NEXT(OpenBlock);
    }
    ...
    PUNICODE_STRING RegistryPath)
{
    ...
    Globals.ProtocolReceiveNetBufferListsOriginal = *NDIS_PROTOCOL_BLOCK_RECVNBL(Globals.TcpIpProtocolBlock);
