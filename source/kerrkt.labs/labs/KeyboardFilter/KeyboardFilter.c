    ...
    PUNICODE_STRING RegistryPath )
{
    ...
    {
        DriverObject->MajorFunction[Index] = FilterDispatch;
    }
    ...
    if (!NT_SUCCESS(Status = IoCreateDevice(DriverObject, sizeof(*DeviceExtension), NULL, FILE_DEVICE_KEYBOARD, FILE_AUTOGENERATED_DEVICE_NAME, TRUE, &FilterDeviceObject)))
    {
    ...
    RtlInitUnicodeString(&KeyboardDeviceName, L"\\Device\\KeyboardClass1");
    ...
    if (!NT_SUCCESS(Status = IoAttachDevice(FilterDeviceObject, &KeyboardDeviceName, &DeviceExtension->StackDeviceObject)))
    {
        DbgPrint("ERROR IoAttachDevice (%x)\n", Status);
        goto Exit;
    }
